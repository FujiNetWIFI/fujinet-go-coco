#!/usr/bin/env bash
#
# Build the FujiNet CoCo runtime as a shared library (libfujinet.so) plus its
...
#   * Source is the user's LOCAL fujinet-pc-coco checkout (not a pinned GitHub
#     tarball), staged into work/fujinet-firmware so the checkout is untouched.
#   * The PC build targets COCO (build.sh -cp COCO), whose DRIVEWIRE bus uses the
#     Becker port (DriveWire-over-TCP) transport. FujiNet's BeckerSocket *listens*
#     (default _listening=true) on the [BOIP] host/port, and XRoar's becker client
#     connects out to it.
#   * fnconfig.ini is forced to [BOIP] enabled=1 host=127.0.0.1 port=65504 (the
#     COCO build reads the becker listen endpoint from the [BOIP] host/port keys)
#     so the in-process emulator connects to the in-process runtime on loopback.
#   * The fujinet web admin UI is bound to port 8002 by fujinet_android_entry.cpp
#     (the -u 0.0.0.0:8002 arg), which the app's WebView points at.
#
...
# transforms to the staged copy -- the same fujinet-pc codebase as the APPLE
# build, so the transforms are shared verbatim.
...
# Local fujinet-pc-coco checkout (override with FUJINET_SRC=/path ...).
FUJINET_SRC="${FUJINET_SRC:-${HOME}/Workspace/fujinet-pc-coco}"
PC_TARGET="COCO"
...
Builds the FujiNet COCO Android runtime from the local fujinet-pc-coco checkout.
EOF
}
...
stage_local_source() {
    [[ -d "${FUJINET_SRC}" ]] || fail "fujinet-pc-coco source not found at ${FUJINET_SRC} (set FUJINET_SRC)"
...
# Android app the emulator shares the process and may mutate the CWD (XRoar's
# path handling -> chdir), so root "data" absolutely from the
...
# failing task. fujinet-pc-coco may not ship this PC FreeRTOS shim (the COCO
...
# NOTE: The COCO DRIVEWIRE bus uses the Becker port (plain DriveWire-over-TCP,
# BeckerSocket), so the ADAM-specific AdamNet BoIP response-deadline fix does
# not apply here -- no transport transform is needed.
...
force_boip_config() {
    # Ensure the runtime's Becker port (DriveWire-over-TCP) listens on loopback
    # for the in-process XRoar emulator to connect to. The COCO build's DRIVEWIRE
    # bus reads the becker listen endpoint from the [BOIP] host/port keys, and
    # BeckerSocket defaults to listening (_listening=true).
...
echo "FujiNet COCO Android runtime outputs updated:"
